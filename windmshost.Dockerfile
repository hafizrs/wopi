...
RUN dotnet restore ./DmsConversionHostService/

RUN dotnet build ./DmsConversionHostService/


#   publish
RUN dotnet publish ./DmsConversionHostService/ -o /publish --configuration Release
RUN ls /publish
...
ENTRYPOINT ["dotnet", "DmsConversionHostService.dll"]
