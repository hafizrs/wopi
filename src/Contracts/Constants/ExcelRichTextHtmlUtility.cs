        ...
        public static void SetRichTextFromHtml(ExcelRange range, string html, string defaultFontName, short defaultFontSize)
        {
            ...
            foreach (Match m in Regex.Matches(html, @"<(/?[a-z]+)[^<>]*>([\s\S]*?)(?=</?[a-z]+[^<>]*>|$)", RegexOptions.Compiled | RegexOptions.IgnoreCase))
            {
                if (isFirst)
                {
                    ...
                    thisrt = range.RichText.Add("start"); // May be 0-length
            ...
            else if (string.IsNullOrWhiteSpace(thisrt.Text) || thisrt.Text == "start")
            {
            ...
            Console.WriteLine($"Rich text for current range = {range.Value}");
