...
{
    public class GetEquipementClientQueryHandler : IQueryHandler<GetEquipementClientQuery, QueryHandlerResponse>
    {
        private readonly ILogger<GetEquipementClientQueryHandler> _logger;
        private readonly IPraxisEquipmentQueryService _service;

        public GetEquipementClientQueryHandler(
            ILogger<GetEquipementClientQueryHandler> logger,
            IPraxisEquipmentQueryService service)
        {
            _logger = logger;
            _service = service;
        }
        public QueryHandlerResponse Handle(GetEquipementClientQuery query)
        {
        ...
        public async Task<QueryHandlerResponse> HandleAsync(GetEquipementClientQuery query)
        {
            if (query == null)
            {
            ...
            try
            {
                var queryResult = await _service.GetPraxisClientsForEquipement(query);
