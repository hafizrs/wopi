...
{
    public class GetEquipementUserQueryHandler : IQueryHandler<GetEquipementUserQuery, QueryHandlerResponse>
    {
        private readonly ILogger<GetEquipementQueryHandler> _logger;
        private readonly IPraxisEquipmentQueryService _service;

        public GetEquipementUserQueryHandler(
            ILogger<GetEquipementQueryHandler> logger,
            IPraxisEquipmentQueryService service)
        {
            _logger = logger;
            _service = service;
        }
        public QueryHandlerResponse Handle(GetEquipementUserQuery query)
        {
        ...
        public async Task<QueryHandlerResponse> HandleAsync(GetEquipementUserQuery query)
        {
            if (query == null)
            {
            ...
            try
            {
                var queryResult = await _service.GetPraxisUsersForEquipement(query);
            ...
            catch (Exception ex)
            {
                _logger.LogError(ex, "Exception in {HandlerName}. Exception Message: {Message}. Exception Details: {StackTrace}.",
                    nameof(GetEquipementUserQueryHandler), ex.Message, ex.StackTrace);
            }
