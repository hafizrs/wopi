...
{
    public class GetEquipementClientCategoryQueryHandler : IQueryHandler<GetEquipementClientCategoryQuery, QueryHandlerResponse>
    {
        private readonly ILogger<GetEquipementClientCategoryQueryHandler> _logger;
        private readonly IPraxisEquipmentQueryService _service;

        public GetEquipementClientCategoryQueryHandler(
            ILogger<GetEquipementClientCategoryQueryHandler> logger,
            IPraxisEquipmentQueryService service)
        {
            _logger = logger;
            _service = service;
        }
        public QueryHandlerResponse Handle(GetEquipementClientCategoryQuery query)
        {
        ...
        public async Task<QueryHandlerResponse> HandleAsync(GetEquipementClientCategoryQuery query)
        {
            if (query == null)
            {
            ...
            try
            {
                var queryResult = await _service.GetPraxisClientCategoryForEquipement(query);
