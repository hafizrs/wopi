...
{
    public class GetEquipementQueryHandler : IQueryHandler<GetEquipementQuery, QueryHandlerResponse>
    {
        private readonly ILogger<GetEquipementQueryHandler> _logger;
        private readonly IPraxisEquipmentQueryService _service;

        public GetEquipementQueryHandler(
        ...
        public QueryHandlerResponse Handle(GetEquipementQuery query)
        {
        ...
        public async Task<QueryHandlerResponse> HandleAsync(GetEquipementQuery query)
        {
            if (query == null)
            {
            ...
            try
            {
                var queryResult = await _service.GetPraxisEquipments(query);
