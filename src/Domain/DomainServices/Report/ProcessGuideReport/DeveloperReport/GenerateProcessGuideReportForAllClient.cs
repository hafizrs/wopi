...
{
    public class GenerateProcessGuideReportForAllClient : IProcessGuideReportGenerate
    {
        private readonly ILogger<GenerateProcessGuideReportForAllClient> _logger;
        ...
        public GenerateProcessGuideReportForAllClient(
            ILogger<GenerateProcessGuideReportForAllClient> logger,
        ...
        public async Task<bool> GenerateReport(ExcelPackage excel, ExportProcessGuideReportForDeveloperCommand command)
        {
            ...
            WriteHeader(worksheet, command.ReportHeader, command.ClientName);
        ...
        {
            const int tableColumns = ProcessGuideDeveloperReport.ColumnsForAllDataReport;
            for (var i = 1; i <= tableColumns; i++)
            {
                var column = worksheet.Column(i);
                var headerCell = worksheet.Cells[ProcessGuideDeveloperReport.HeaderRowIndexForAllDataReport, i];
                column.AutoFit();
        ...
        private void WriteReportData(ExcelWorksheet workSheetTable, List<DeveloperForProcessGuideReport> reportDataList)
        {
            try
            {
                var rowIndex = ProcessGuideDeveloperReport.HeaderRowIndexForAllDataReport + 1;
                var clientIndex = 0;
            ...
            {
                _logger.LogError(ex, "Exception occurred during writing report data to generate Excel report. Exception message: {Message}. Exception Details: {StackTrace}", ex.Message, ex.StackTrace);
            }
        }
                ...
                foreach (var task in form.ProcessGuideCheckList)
                {
                    ...
                    foreach (var processGuideTask in task.ProcessGuideTask)
                    {
                        var taskDescription = new TaskDescriptionDto()
                        {
                            ...
                            Budget = processGuideTask.Budget.GetValueOrDefault()
                        };
        ...
        private void WriteHeader(ExcelWorksheet worksheet, string reportName, string clientName)
        {
            try
            {
                ...
                var columnIndex = 1;
                const int headerRowIndex = ProcessGuideDeveloperReport.HeaderRowIndexForAllDataReport;
        ...
        {
            const int logoStartColumn = ProcessGuideDeveloperReport.ColumnsForAllDataReport;
            // [firstRow, firstColumn, lastRow, LastColumn]
