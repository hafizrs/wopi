...
{
    public class ShiftTaskAssignService : IShiftTaskAssignService
    {
        private readonly IRepository _repository;
        private readonly ILogger<ShiftTaskAssignService> _logger;
        ...
        public ShiftTaskAssignService(
            IRepository repository,
            ILogger<ShiftTaskAssignService> logger,
        ...
        public async Task AssignTasks(RiqsShiftPlan shiftPlan)
        {
            List<string> formIds = shiftPlan.Shift.PraxisFormIds;
            string departmentId = shiftPlan.Shift.DepartmentId;
            ...
            foreach (var formId in formIds)
            {
                try
                {
                    ...
                    PraxisShift shift = new PraxisShift
                    {
                        ItemId = shiftPlan.Shift.ItemId,
                        Name = shiftPlan.Shift.ShiftName,
                    };

                    if (processGuide == null)
                    {
                        await AssignTask(shift, formId, departmentId, shiftPlan.ItemId);
                    }
                    else
                    {
                        await UpdateProcessGuide(shift, processGuide, shiftPlan);
                        await UpdateShiftPlanForProcessGuideCreated(new List<string> { shiftPlan.ItemId }, processGuide.ItemId);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex.Message);
                    _logger.LogError("Failed to create processguide with FormId -> {formId} and DepartmentId -> {departmentId}", formId, departmentId);
                }
            }

        }

        public async Task UpdateShiftPlanForProcessGuideCreated(List<string> shiftPlanIds, string processGuideId)
        {
            var updates = new Dictionary<string, object>
            {
                { "IsProcessGuidCreated", true },
                { "ProcessGuideId", processGuideId }
            };
            await _repository.UpdateAsync<RiqsShiftPlan>(sp => shiftPlanIds.Contains(sp.ItemId), updates);
        }

        private async Task UpdateProcessGuide(PraxisShift shift, PraxisProcessGuide processGuide, RiqsShiftPlan plan)
        {
            var controllMembers = (List<string>)processGuide.ControlledMembers;
            controllMembers.AddRange(plan.PraxisUserIds ?? new List<string>());
        ...
        private async Task AssignTask(PraxisShift shift, string formId, string departmentId, string shiftPlanId)
        {
            ...
            var taskSchedulerModel = GetTaskSchedulerModel(processGuideConfig, praxisForm.Description, new List<PraxisShift> { shift }, shiftPlanId);
            ...
            {
                _logger.LogError("Exception: {Message} StackTrace: {StackTrace} Failed to create process guide with processGuideConfigId -> {ItemId}", 
                    ex.Message, ex.StackTrace, processGuideConfig.ItemId);
            }
        }
        private List<string> GetAssignedUserIds(string formId, string departmentId)
        {
            ...
            var userIds = _repository.GetItems<RiqsShiftPlan>(sp => sp.ShiftDate >= utcDate && sp.ShiftDate <= utcDate && sp.Shift.PraxisFormIds.Contains(formId) && sp.Shift.DepartmentId == departmentId)
                .SelectMany(sp => sp.PraxisUserIds).Distinct().ToList();
            return userIds;
        }
        ...
        private async Task<PraxisProcessGuideConfig> SaveNewProcessGuideConfig(string formId, PraxisForm praxisForm, string departmentId)
        {
            ...
            var subissionDates = new List<DateTime>();
            subissionDates.Add(utcDate);
        ...
        private CreateTaskScheduleRequestModel GetTaskSchedulerModel(PraxisProcessGuideConfig processGuideConfig, string formName, List<PraxisShift> shifts, string shiftPlanId)
        {
            ...
            var taskData = new TaskData()
            {
                ...
                RelatedEntityName = "PraxisProcessGuide",
                TaskSummaryId = Guid.NewGuid().ToString(),
                Title = processGuideConfig.Title,
                RelatedEntityObject = GetRelatedEngityObject(processGuideConfig, formName, shifts, shiftPlanId)
            };
            taskDatas.Add(taskData);

            var submissionDates = new List<string>();
            submissionDates.Add(DateTime.UtcNow.ToString("yyyy-MM-dd"));
        ...
        private RelatedEntityObject GetRelatedEngityObject(PraxisProcessGuideConfig processGuideConfig, string formName, List<PraxisShift> shifts, string shiftPlanId)
        {
            return new RelatedEntityObject()
            {
                ...
                Shifts = shifts,
                RelatedEntityId = shiftPlanId,
                RelatedEntityName = nameof(RiqsShiftPlan)
            };
        }
