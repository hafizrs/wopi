...
{
    public class GenerateQuickTaskReportService : IGenerateQuickTaskReportService
    {
        private Dictionary<string, string> _translatedStrings = new Dictionary<string, string>();

        private readonly ILogger<GenerateQuickTaskReportService> _logger;
        ...
        public GenerateQuickTaskReportService(
            ILogger<GenerateQuickTaskReportService> logger,
        ...
        public bool GenerateQuickTaskReport(ExcelPackage excel, GenerateQuickTaskReportCommand command)
        {
            _logger.LogInformation("Entered into Service: {ServiceName} with command: {Command}",
                nameof(GenerateQuickTaskReportService), JsonConvert.SerializeObject(command));
            try
            {
                GetTranslations(command.LanguageKey);
                var worksheet = excel.Workbook.Worksheets.Add(command.FileName);
                WriteQuickTaskReportHeader(worksheet, command);
                WriteQuickTaskReportTableData(worksheet, command);
                return true;
            }
            catch (Exception e)
            {
                _logger.LogError("Error in Service: {ServiceName} Error Message: {Message} Error Details: {StackTrace}",
                    nameof(GenerateQuickTaskReportService), e.Message, e.StackTrace);
                return false;
            }
        }

        public void SetupRolesForQuickTaskReport(PraxisReport report, GenerateQuickTaskReportCommand command)
        {
        ...
        private void GetTranslations(string languageKey)
        {
            ...
            keyList.AddRange(ShiftReport.LanguageKeys.Values); // You may want to define QuickTaskReport keys
            _translatedStrings = _uilmResourceKeyService.GetResourceValueByKeyName(keyList, languageKey);
        }

        private void WriteQuickTaskReportHeader(ExcelWorksheet worksheet, GenerateQuickTaskReportCommand command)
        {
            ...
            var columnIndex = 1;
            foreach (var primaryTableColumnKey in ShiftReport.PrimaryTableColumnKeys) // You may want to define QuickTaskReport columns
            {
        ...
        private void WriteQuickTaskReportTableData(ExcelWorksheet worksheet, GenerateQuickTaskReportCommand command)
        {
            var quickTasks = _quickTaskService.GetQuickTasks(command.ClientId);
            if (!string.IsNullOrWhiteSpace(command.SearchText))
            {
                quickTasks = quickTasks
                    .Where(x => x.TaskGroupName.ToLower().Contains(command.SearchText.ToLower()))
                    .ToList();
            }
            ...
            {
                // first column
                worksheet.Cells[rowIndex, 1].Value = quickTask.TaskGroupName;
                ...
                // second column
                worksheet.Cells[rowIndex, 2].Value = GetTaskTitles(quickTask);
                ...
                // third column
                worksheet.Cells[rowIndex, 3].Value = GetAttachedDocuments(quickTask);
                ...
                // fourth column
                worksheet.Cells[rowIndex, 4].Value = GetAttachedForms(quickTask);
        ...
        private void AddHeaderLogo(ExcelWorksheet worksheet, int columnIndex)
        {
            ...
            _praxisReportService.AddLogoInExcelReport(worksheet, LibraryReport.LogoSize, columnIndex, null);
        }

        private string GetTaskTitles(RiqsQuickTaskResponse quickTask)
        {
            if (quickTask.TaskList == null) return string.Empty;
            var titles = quickTask.TaskList.Select(t => t.Title).ToList();
            return string.Join(", ", titles);
        }

        private string GetAttachedDocuments(RiqsQuickTaskResponse quickTask)
        {
            if (quickTask.TaskList == null) return string.Empty;
            var fileNames = quickTask.TaskList.SelectMany(t => t.Files?.OfType<dynamic>().Select(f => f?.DocumentName?.ToString()) ?? new List<string>()).ToList();
            return string.Join(", ", fileNames);
        }
        private string GetAttachedForms(RiqsQuickTaskResponse quickTask)
        {
            if (quickTask.TaskList == null) return string.Empty;
            var formNames = quickTask.TaskList.SelectMany(t => t.LibraryForms?.Select(f => f.LibraryFormName) ?? new List<string>()).ToList();
