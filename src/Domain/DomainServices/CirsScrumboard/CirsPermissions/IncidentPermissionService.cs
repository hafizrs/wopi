...
{
    public class IncidentPermissionsService : IPermissionsService
    {
        private readonly ISecurityContextProvider _securityContextProvider;
        private readonly ISecurityHelperService _securityHelperService;

        public IncidentPermissionsService(
        ...
        public Dictionary<string, bool> GetPermissions(CirsDashboardPermission dashboardPermissions)
        {
            ...
            var canInputExternalOfficeEmailKey = CirsModuleConstants.PermissionKeys[$"{CirsPermissionEnum.CAN_INPUT_EXTERNAL_OFFICE_EMAIL}"];
            var canMoveReportForPublishingKey = CirsModuleConstants.PermissionKeys[$"{CirsPermissionEnum.CAN_MOVE_REPORT_FOR_PUBLISHING}"];
            ...
            var canEditPublishedReportKey = CirsModuleConstants.PermissionKeys[$"{CirsPermissionEnum.CAN_EDIT_PUBLISHED_REPORT}"];
            ...
            var hideToBeApprovedColumnKey = CirsModuleConstants.PermissionKeys[$"{CirsPermissionEnum.HIDE_TO_BE_APPROVED_COLUMN}"];
            ...
            var canInputExternalOfficeEmail = CanInputExternalOfficeEmail(isAssignedAdmin, dashboardPermissions.AssignmentLevel);
            var canMoveReportForPublishing = CanMoveReportForPublishing(isAssignedAdmin, dashboardPermissions.AssignmentLevel);
            ...
            var canEditPublishedReport = CanEditPublishedReport(isAssignedAdmin, dashboardPermissions.AssignmentLevel);
            ...
            var hideToBeApprovedColumn = HideToBeApprovedColumn(isAssignedAdmin, dashboardPermissions.AssignmentLevel);
            ...
            permissions.Add(canInputExternalOfficeEmailKey, canInputExternalOfficeEmail);
            permissions.Add(canMoveReportForPublishingKey, canMoveReportForPublishing);
            permissions.Add(canSeeActiveCardsKey, canSeeActiveCards);
            permissions.Add(canSeeInactiveCardsKey, canSeeInactiveCards);
            permissions.Add(canEditPublishedReportKey, canEditPublishedReport);
            permissions.Add(canGenerateExcelReportKey, canGenerateExcelReport);
            permissions.Add(canDeleteReportKey, canDeleteReport);
            permissions.Add(hideToBeApprovedColumnKey, hideToBeApprovedColumn);
        ...
        private bool CanAssignAdmin(AssignmentLevel assignmentLevel)
        {
            ...
            {
                return _securityHelperService.IsAAdminOrTaskConrtroller() || _securityHelperService.IsAGroupAdminUser() ||
        ...
        private bool CanEditReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAGroupAdminUser() || _securityHelperService.IsAAdminBUser() || isAssignedAdmin;
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
        ...
        private bool CanInactiveReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAGroupAdminUser() || isAssignedAdmin || _securityHelperService.IsAAdminBUser();
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
                return isAssignedAdmin || _securityHelperService.IsAPowerUser();
        ...
        private bool CanInputExternalOfficeEmail(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAAdminOrTaskConrtroller() || _securityHelperService.IsAGroupAdminUser() || 
        ...
        private bool CanMoveReportForPublishing(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAAdminBUser() || isAssignedAdmin;
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
        ...
        private bool CanSeeInactiveCards(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            ...
            {
                return _securityHelperService.IsAPowerUser() || isAssignedAdmin || _securityHelperService.IsAAdmin() || _securityHelperService.IsAManagementUser();
            }
            return false;
        }

        private bool CanEditPublishedReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAAdminBUser() || isAssignedAdmin;
        ...
        private bool CanGenerateExcelReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            ...
            {
                return _securityHelperService.IsAAdminOrTaskConrtroller() || isAssignedAdmin || _securityHelperService.IsAPowerUser() || _securityHelperService.IsAManagementUser();
            }
            return false;
        }
        ...
        private bool HideToBeApprovedColumn(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return !(isAssignedAdmin || _securityHelperService.IsAAdmin() || _securityHelperService.IsAAdminBUser() || _securityHelperService.IsAGroupAdminUser() || _securityHelperService.IsAPowerUser());
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
                return !(isAssignedAdmin || _securityHelperService.IsAAdmin() || _securityHelperService.IsAAdminBUser() || _securityHelperService.IsAGroupAdminUser() || _securityHelperService.IsAPowerUser());
            ...
            {
                return !(isAssignedAdmin || _securityHelperService.IsAAdmin() || _securityHelperService.IsAManagementUser() || _securityHelperService.IsAPowerUser());
            }
            return false;
        }
    }
