...
{
    public class IdeaPermissionsService : IPermissionsService
    {
        private readonly ISecurityContextProvider _securityContextProvider;
        private readonly ISecurityHelperService _securityHelperService;

        public IdeaPermissionsService(
        ...
        public Dictionary<string, bool> GetPermissions(CirsDashboardPermission dashboardPermissions)
        {
            ...
            var canRequalifyReportKey = CirsModuleConstants.PermissionKeys[$"{CirsPermissionEnum.CAN_REQUALIFY_REPORT}"];
            ...
            var canRequalifyReport = CanRequalifyReport(isAssignedAdmin, dashboardPermissions.AssignmentLevel);
            ...
            permissions.Add(canRequalifyReportKey, canRequalifyReport);
        ...
        private bool CanAssignAdmin(AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAAdminOrTaskConrtroller() ||
                    _securityHelperService.IsAGroupAdminUser() || _securityHelperService.IsAAdminBUser();
            }
        ...
        private bool CanEditReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return _securityHelperService.IsAAdminBUser() || isAssignedAdmin || _securityHelperService.IsAGroupAdminUser();
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
        ...
        private bool CanViewReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            ...
            {
                return _securityHelperService.IsAAdminOrTaskConrtroller() || isAssignedAdmin || _securityHelperService.IsADepartmentLevelUser();
            }
            return false;
        }
        ...
        private bool CanInactiveReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
                return isAssignedAdmin || _securityHelperService.IsAGroupAdminUser() || _securityHelperService.IsAAdminBUser() || _securityHelperService.IsAAdmin();
            }
            else if (assignmentLevel == AssignmentLevel.UnitWithInsight)
            {
        ...
        private bool CanRequalifyReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            if (assignmentLevel == AssignmentLevel.Organizational)
            {
        ...
        private bool CanSeeInactiveCards(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            ...
            {
                return _securityHelperService.IsAAdmin() || isAssignedAdmin || _securityHelperService.IsADepartmentLevelUser();
            }
            return false;
        }
        ...
        private bool CanGenerateExcelReport(bool isAssignedAdmin, AssignmentLevel assignmentLevel)
        {
            ...
            {
                return !_securityHelperService.IsAMpaUser() && !_securityHelperService.IsAAdminBUser() & !_securityHelperService.IsAGroupAdminUser() || isAssignedAdmin;
            }
            return false;
        }
