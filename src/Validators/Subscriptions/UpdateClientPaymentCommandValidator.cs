...
{
    public class UpdateClientPaymentCommandValidator: AbstractValidator<UpdateClientPaymentCommand>
    {
        public UpdateClientPaymentCommandValidator()
        {
            RuleFor(command => command.OrganizationId)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .NotNull().WithMessage("OrganizationId can't be null.")
                .NotEmpty().WithMessage("OrganizationId can't be empty.")
                .Must(IsValidGuid).WithMessage("Guid is not valid.");
        ...
        public ValidationResult IsSatisfiedby(UpdateClientPaymentCommand command)
        {
